defmodule ALF.Manager.StreamRegistry do
  ...
  def move_to_in_progress_registry(%__MODULE__{} = registry, ips) do
    {inputs, in_progress, decomposed, recomposed} =
      Enum.reduce(
        ips,
        {registry.inputs, registry.in_progress, registry.decomposed, registry.recomposed},
        fn %{in_progress: true} = ip, {inputs, in_progress, decomposed, recomposed} ->
          {Map.delete(inputs, ip.ref), Map.put(in_progress, ip.ref, ip.event), decomposed,
           recomposed}
        end
      )
