defmodule ALF.CloneTest do
  ...
  def setup_consumers(stage_pid) do
    {:ok, consumer1_pid} = TestConsumer.start_link(
      %TestConsumer{subscribe_to: [{stage_pid, max_demand: 1}]}
    )
  ...
  setup %{producer_pid: producer_pid} do
    {:ok, pid} = Clone.start_link(
      build_clone(producer_pid)
    )
  ...
  test "test clone", %{producer_pid: producer_pid, consumer1_pid: consumer1_pid, consumer2_pid: consumer2_pid} do
    ip = %IP{datum: 1}
